...
Unit3PolygonOffset::Unit3PolygonOffset(QWidget *parent)
...
Unit3PolygonOffset::~Unit3PolygonOffset()
{
...
void Unit3PolygonOffset::initializeGL()
{
...
void Unit3PolygonOffset::paintGL()
{
    //清除颜色缓冲区和深度缓冲区
    ...
    //DZ是深度值-z值相对于多边形屏幕区域的变化量，而r是使深度产生变化的最小值
    //负值将使z值距离我们更近，而正值移得更远
    //除了偏移还必须启用多边性单独偏移来填充，如GL_POLYGON_OFFSET_LINE
...
void Unit3PolygonOffset::resizeGL(int width, int height)
{
    //视口，靠左下角缩放
...
void Unit3PolygonOffset::keyPressEvent(QKeyEvent *event)
{
    //左右转围绕的是y轴，上下转围绕的是x轴
    switch (event->key()) {
    case Qt::Key_Up:
...
void Unit3PolygonOffset::showEvent(QShowEvent *event)
{
    setFocus();
